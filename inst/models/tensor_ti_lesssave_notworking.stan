...
model {
  ...
  for (m in 1:N_pos) {
    ...
    real rate_param;
     ...
     mean_param = b_0_mean + X_mean_jdate[m] * b_mean_jdate + X_mean_year[m] * b_mean_year + X_mean_tensor[m] * b_mean_tensor + mu_err;
     disp_param = -7 + log(1 + exp(b_0_disp + X_disp_jdate[m] * b_disp_jdate + X_disp_year[m] * b_disp_year + X_disp_tensor[m] * b_disp_tensor)) + phi_err;

    shape_param = inv( exp(disp_param ) ) ;
    rate_param = inv( exp(mean_param ) * exp(disp_param) ) ;

    y_pos[m] ~ gamma(shape_param, rate_param);   // 
  }
  ...
  for (j in 1:N) {
    ...
    real theta_param;
     ...
     theta = b_0_theta + X_theta_jdate[j] * b_theta_jdate + X_theta_year[j] * b_theta_year + X_theta_tensor[j] * b_theta_tensor; 

    theta_param ~ normal(theta, sigma_theta);

    y_zero[j] ~ bernoulli_logit(theta_param);   // 
  }
  ...
  //sigma_theta ~ gamma(sigma_theta_prior[1,1], sigma_theta_prior[1,2]);
   ...
   mu_err ~ normal(0, sigma_mean);
   phi_err ~ normal(0, sigma_disp);
}
